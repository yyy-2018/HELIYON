# -*- coding: utf-8 -*-
"""
Created on Sun Jun 30 00:42:44 2024
...
def modelws(seed_numble):
    ...
    sbs = t.addMVar(shape=20,lb=-float("inf"),name="sbs")
    qs = t.addVar(name="qs")
    ...
    t.setObjective(c@e+qs,GRB.MINIMIZE)
    ...
    t.addConstrs(-op[i]+oc1<=ee for i in range(20))
    t.addGenConstrNorm(qs,sbs,2.0, "normconstr")
    t.addConstrs(e[i]*cbo[i] == sbs[i] for i in range(20))
    ...
    m.update()
    dsjlist.append(-round(sum(-c[i]*x[i].x for i in range(20)),2))
    msjlist.append(sum(c[i]*e[i].x for i in range(20))+qs.x)
    # print(-round(m.ObjVal,3))
    # print(-round(sum(l[i]*y[i].x for i in range(20)),2))
    # print(-round(sum(-c[i]*x[i].x for i in range(20)),2))
    # # print(sum(c[i]*e[i].x for i in range(20))+qs.x)
    # print(round(oc1.x,3))
