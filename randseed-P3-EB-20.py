# -*- coding: utf-8 -*-
"""
Created on Sun Jun 30 00:39:48 2024
...
def modelws(seed_numble):
    ...
    #sbs3 = t.addMVar(shape=20,lb=-float("inf"),name="sbs")
    ...
    #t.addConstrs(e[i]*cbo[i] == sbs3[i] for i in range(20))
    ...
    try:
        ...
        sbs = m.addMVar(shape=20,lb=-float("inf"),name="sbs")
        qs = m.addVar(name="qs")
        sbs2 = m.addMVar(shape=20,lb=-float("inf"),name="sbs2")
        ...
        m.addConstrs(x[i]*cbo[i] == sbs2[i] for i in range(20))
        
        m.addGenConstrNorm(qs,sbs,2.0, "normconstr2")
        m.addConstrs(y[i]*lbo[i] == sbs[i] for i in range(20))
    ...
    dsjlist.append(-round(sum(-c[i]*x[i].x+cbo[i]*x[i].x for i in range(20)),2))
    msjlist.append(round(sum(c[i]*e[i].x+cbo[i]*e[i].x for i in range(20)),2))
    # print(-round(m.ObjVal,3))
    # print(-round(sum(l[i]*y[i].x for i in range(20))+qs.x,2))
    # print(-round(sum(-c[i]*x[i].x for i in range(20))+qs2.x,2))
    # # print(sum(c[i]*e[i].x+e[i].x*cbo[i] for i in range(20)))
    # print(round(oc1.x,3))
